...
int i;
int len = 0;
char **tokens = NULL, *path_env = NULL,
*temporary = NULL, *env_var = environ[0];
while (env_var[len] != '=')
{
len++;
}
...
for (i = 0; tokens[i]; i++)
{
temporary = malloc((_strlen(tokens[i])
+ _strlen(command[0]) + 2 + 1) * sizeof(char));
...
free_arr(tokens);
if (temporary != NULL && (_strcmp(command[0],
"ls") == 0 || _strcmp(command[0],
"echo") == 0 || _strcmp(command[0], "rm") == 0))
{
free(command[0]);
command[0] = temporary;
return;
